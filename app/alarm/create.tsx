...
// Local type definitions (mirrors src/missions/types.ts)
...
// ---------------------------------------------------------------------------
// SnoozeChip — animated preset chip
// ---------------------------------------------------------------------------
...
function DatePickerModal({ visible, initial, onConfirm, onCancel, colors }: DatePickerModalProps) {
  ...
  // Clamp day if month/year changes reduces available days
      ...
      <View style={{ width, height: ITEM_H * 5, overflow: 'hidden' }}>
        ...
        >
          {items.map((item, i) => {
            ...
            return (
              <View key={i} style={[dpStyles.item, { height: ITEM_H }]}>
                <Text style={[dpStyles.itemText, { color: isSelected ? colors.primary : colors.onSurfaceVariant, fontWeight: isSelected ? '700' : '400' }]}>
                  {item}
                </Text>
              </View>
            );
        ...
        {/* Selection highlight */}
        <View pointerEvents="none" style={[dpStyles.selector, { borderColor: colors.primaryDim }]} />
      </View>
    );
  }
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onCancel}>
      <Pressable style={dpStyles.overlay} onPress={onCancel}>
        <Pressable style={[dpStyles.panel, { backgroundColor: colors.surfaceContainer }]} onPress={() => {}}>
          <Text style={[dpStyles.title, { color: colors.onSurface }]}>SELECT DATE</Text>
          ...
          <View style={dpStyles.actions}>
            <Pressable onPress={onCancel} style={[dpStyles.btn, { borderColor: colors.outlineVariant }]}>
              <Text style={[dpStyles.btnText, { color: colors.onSurfaceVariant }]}>Cancel</Text>
            </Pressable>
            <Pressable
              onPress={() => onConfirm(new Date(selYear, selMonth, clampedDay))}
              style={[dpStyles.btn, { backgroundColor: colors.primaryDim, borderColor: colors.primaryDim }]}
            >
              <Text style={[dpStyles.btnText, { color: '#FFFFFF' }]}>Confirm</Text>
            </Pressable>
          </View>
        </Pressable>
      </Pressable>
...
export default function CreateAlarmScreen() {
  ...
  const [customSnoozeInput, setCustomSnoozeInput] = useState('5');
  const [missions, setMissions] = useState<AlarmMission[]>([]);
  ...
  function handleSave() {
    const id = `alarm_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const now = Date.now();
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { borderBottomColor: colors.outlineVariant + '33' }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.onSurface }]}>NEW ALARM</Text>
        <Pressable onPress={handleSave} style={styles.headerBtn}>
          <Text style={[styles.headerBtnTextRight, { color: colors.primary }]}>Save</Text>
        </Pressable>
      </View>
      ...
      <View style={[styles.ctaContainer, { backgroundColor: colors.background }]}>
        <Pressable onPress={handleSave} style={styles.ctaPressable}>
          ...
          >
            <Text style={styles.ctaText}>SET ALARM</Text>
          </LinearGradient>
        </Pressable>
        <SafeAreaView />
      </View>
...
// Section header with label + horizontal line
...
const styles = StyleSheet.create({
  ...
  // Date chip (matches DayPicker chip style)
